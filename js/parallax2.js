...
jQuery(function($) {
    ...
    var setLogo = function() {
        $('.site-main .header-wrp , .featured-image .header-wrp , .site-footer .header-wrp').each(function() {
            $(this).css('top', $('.site-header').offset().top - $(this).parent().offset().top)
        })
    };
    ...
    $(document).ready(function() {
        ...
        $('.mobile-nav-btn-1 , .mobile-nav-btn-2').click(function() {
            $(this).toggleClass('open')
        });
        $('.mobile-nav-btn , .nav-overlay').on('click', function() {
            $('.mobile-nav-btn').toggleClass('open');
            $('.navigation-wrap').toggleClass('slide-left');
            $('.nav-overlay').fadeToggle(300)
        });
        $(window).scroll(function() {
            if ($(window).scrollTop() >= 150) {
                $('.site-header').addClass('afterscroll');
                $('.scrolldown-btn-wrap').addClass('afterscroll')
            } else {
                $('.site-header').removeClass('afterscroll');
                $('.scrolldown-btn-wrap').removeClass('afterscroll')
            }
        });
        $(window).scroll(function() {
            var TotalHeight = $(window).height();
            if ($(window).scrollTop() >= TotalHeight) {
                $('.single-project .site-header').addClass('showme')
            } else {
                $('.single-project .site-header').removeClass('showme')
            }
        });
        ...
        $('.site-main .clients-section .logos-wrap ul li .img-wrap').matchHeight();
            ...
            if (jQuery(e.target).is('.active')) {
                ...
                jQuery(this).closest('.module-accordion').find(currentAttrValue).slideDown(300, function() {
                    jQuery('html, body').animate({
                        scrollTop: jQuery('.site-main .main-content-wrap .main-content .module-accordion .accordion-section .accordion-section-title.active').offset().top - headerHeight
                    }, 800)
                }).addClass('open');
                top_reload_slider()
            }
        ...
        if (jQuery(".featured-text-section").length == 0) {
            jQuery('.site-main').addClass('gap-top')
        }
    ...
    $(window).load(function() {
        $.fn.logoresize = function() {
            console.log(jQuery('.single .featured-image .text-wrap .logo-wrap img').length);
            $('.single .featured-image .text-wrap .logo-wrap img').each(function() {
        ...
        $('.single .featured-image .text-wrap .logo-wrap img').logoresize();
        $('.single .featured-image .text-wrap .logo-wrap img').addClass('show-me')
    })
})
